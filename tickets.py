...
def hkdf(secret: bytes, salt: bytes, length: int) -> bytes:
    return HKDF(algorithm=hashes.SHA256(), length=length, salt=salt, info=b"").derive(
        secret
    )
...
def compute_access_ticket(
    cli_priv: X25519PrivateKey,
    ticket_pub: X25519PublicKey,
    ticket_key_id: bytes,
...
def build_payload(
    cli_pub32: bytes,
    ticket_key_id8: bytes,
    nonce32: bytes,
    access_ticket32: bytes,
    pad_len: int,
) -> bytes:
...
class TicketVerifier:
    ...
    def parse_and_verify(self, payload_b64: str, client_ip: str) -> bool:
        ...
        for hb in (
            ...
            salt = sha256(
                TICKET_LABEL + self.ticket_key_id8 + int(hb).to_bytes(8, "big")
            )
...
def generate_client_payload(params: TicketParams) -> Tuple[bytes, bytes, bytes]:
    ...
    access, _ = compute_access_ticket(
        cli_priv,
        X25519PublicKey.from_public_bytes(params.ticket_pub),
        params.ticket_key_id,
    )
